...
public class JobQz {
	...
	// 게임 시작
	public void JobQ(TimerStart TS,String id) {
		...
		while (true) {
			 ...
			 if (point >= 0) {
				...
				while (true) {
					...
					ArrayList<ThemaDTO> list = gdao.jobQZ();
					String word1 = list.get(num).getJob();
					int mult = mul.mulCheck(word1);

					String word = list.get(num).getJob().toUpperCase();
